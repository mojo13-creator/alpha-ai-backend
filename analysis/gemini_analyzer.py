...
Provides a second independent AI opinion for the composite scorer.
Uses the same data package and conviction-scoring prompt as Claude.
"""
...
class GeminiAnalyzer:
    """Gemini-powered stock analyzer — provides an independent AI Insight score."""
    ...
    def _init_client(self):
        """Initialize the Gemini client. Fails silently if key not set."""
        try:
            api_key = os.environ.get("GEMINI_API_KEY", "")
            if not api_key:
                print("⚠️  No Gemini API key configured — Gemini analysis disabled")
                return
    ...
    def get_gemini_insight_score(self, symbol, price, stock_info, technical_result,
                                 fundamental_result, sentiment_result, news_headlines,
                                 df=None, berkeley_data=None, user_period=None,
                                 sec_data=None):
        """
        Call Gemini API for an independent AI Insight score.
        Returns dict matching the same schema as Claude's AI Insight result.
    ...
    def _build_prompt(self, symbol, price, stock_info, technical, fundamental,
                      ...
                      sec_data=None):
        """Build the decisive analyst prompt with all data — mirrors Claude's prompt."""
...
ROLE — BEAR CASE ANALYST (1 of 3 models in consensus):
You are the bear-case lens. Your job: find the strongest credible reason this stock could underperform or break down over the user's horizon. Force yourself to enumerate failure modes BEFORE looking at the score range — what could go wrong? What is the market missing on the downside? Then test those failure modes against the data: do they actually apply, or are they hypothetical?

CRITICAL — score direction is NORMAL, not inverted:
  Score 0-30 = bear case is overwhelming, this stock is broken
  Score 70+ = bear case dies under scrutiny, you couldn't find a credible thesis
  Score 75+ means "I tried to be the bear and failed"

You are NOT being asked to be a permabear. You are being asked to be the model that prosecutes the bear thesis aggressively. Two other models (bull, synth) are doing the same from other angles. The user gets the consensus. If the bear case is hollow, say so plainly and score high.
